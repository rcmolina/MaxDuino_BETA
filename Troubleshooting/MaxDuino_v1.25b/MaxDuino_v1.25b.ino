#define VERSION "v1.25b MaxDuino"
// ---------------------------------------------------------------------------------
...
*/
 ...
 //      delay(500);
/*
       if (blockID= TAP) bytesRead=blockOffset;
       else
*/
...
/*       if (block > 1) block--;
       ...
       bytesRead=blockOffset[block];                                //offset to start file
*/
/*
...
void PlayBlock()
{
       printtextF(PSTR("       "),0);
       #ifdef LCDSCREEN16x2
//         lcd.setCursor(8,0);lcd.print('#'+block);
       #endif
       #ifdef OLED1306
          setXY(0,0);utoa(blockOffset[block],PlayBytes,10);sendStr(PlayBytes);
          setXY(8,0); utoa(blockID[block],PlayBytes,16);sendStr("ID:");sendStr(PlayBytes); // Block ID en hex
       #endif      
       #ifdef P8544
//          lcd.setCursor(12,3);lcd.print('#'+block);
       #endif
...
//  bytesRead=0;                                //start of file
  ...
  checkForEXT (sfileName);                     // change currentTask
  if (!casduino) {
    currentBlockTask = READPARAM;               //First block task is to read in parameters
...
//    Timer1.setPeriod(1000);                     //set 1ms wait at start of a file. Needs digitalWrite
  }
