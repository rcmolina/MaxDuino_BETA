...
void UniPlay(char *filename){
  ...
  if(!entry.open(filename,O_READ)) {
    lcd_clearline(0);
    printtextF(F("Error Opening File"));
  }
...
void TZXStop() {
  ...
  entry.close();                              //Close file
                                                                                // DEBUGGING Stuff
  //lcd.setCursor(0,1);
  //printtextF(blkchksum,HEX); printtextF(F("ck ")); printtextF(bytesRead); printtextF(F(" ")); printtextF(ayblklen);
      ...
      /*  //Old ID20
        ...
        default:
          ...
          //ID Not Recognised - Fall back if non TZX file or unrecognised ID occurs
            ...
            printtextF(F("ID? "));
            lcd.setCursor(4,0);
            printtextF(String(currentID, HEX));
            lcd.setCursor(0,1);
            printtextF(String(bytesRead,HEX) + F(" - L: ") + String(loopCount, DEC));
          #endif
              ...
              u8g.firstPage();
              do {  
                  u8g.drawStr ( 0, 15, F("ID? "));
                  u8g.setPrintPos(40, 15); 
                  u8g.print(String(currentID, HEX));
                  u8g.setPrintPos(0, 30); 
                  u8g.print(String(bytesRead,HEX) + F(" - L: ") + String(loopCount, DEC));
              } while( u8g.nextPage() );
          #endif 
  ...
  switch (currentBlockTask) {
    ...
    case PAUSE:
      //Close block with a pause
        // DEBUG
        //lcd.setCursor(0,1);
        //printtextF(blkchksum,HEX); printtextF(F("ck ptr:")); printtextF(hdrptr);
...
void ReadTZXHeader() {
  ...
  if(entry.seekSet(0)) {
    ...
    if(memcmp_P(tzxHeader,TZXTape,7)!=0) {
      lcd_clearline(1);
      printtextF(F("Not TZXTape"));     
      TZXStop();
    }
  } else {
    lcd_clearline(0);
    printtextF(F("Error Reading File"));        
  }
  bytesRead = 10;
}
