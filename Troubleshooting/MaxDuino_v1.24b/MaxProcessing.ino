...
void TZXProcess() {
    ...
    if(currentTask == GETID) {
      ...
      if(ReadByte(bytesRead)==1) {
        currentID = outByte;
     //   blockOffset[block] = bytesRead-1;
     //   block++;  // points to next block
      } else {
        currentID = EOF;
      }
...
//        count=LONG_HEADER; 
        count = pilotPulses; // antes bytes ahora pulsos
        fileStage+=1;
      } else 
      {
...
*/
      if(currentPeriod>0) {
        ...
        //wbuffer[btemppos+1][workingBuffer ^ 1] = lowByte(currentPeriod); 
/*
        wbuffer[btemppos][workingBuffer ^ 1] = 2;
        wbuffer[btemppos+1][workingBuffer ^ 1] = 2;
*/        
        interrupts();
        //btemppos+=1;
        btemppos+=2;        
