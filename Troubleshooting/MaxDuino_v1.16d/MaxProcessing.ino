    ...
    if(btemppos<=buffsize){                    // Keep filling until full
      ...
      if(currentPeriod>0) {
       ...
       //wbuffer[btemppos][workingBuffer ^ 1] = currentPeriod;   //add period to the buffer
         wbuffer[btemppos][workingBuffer ^ 1] = currentPeriod % 256;   //add period LSB to the buffer      
         wbuffer[btemppos+1][workingBuffer ^ 1] = currentPeriod /256;   //add period MSB to the buffer       
        interrupts();
        //btemppos+=1;
        btemppos+=2;    
      }
       ...
       /*  //Old ID20
        ...
        default:
          ...
          //ID Not Recognised - Fall back if non TZX file or unrecognised ID occurs
            ...
            lcd.setCursor(0,1);
            lcd.print(String(bytesRead,HEX) + F(" - L: ") + String(loopCount, DEC));
          #endif
...
void wave2() {
  ...
  word workingPeriod = wbuffer[pos][workingBuffer] + 256 * wbuffer[pos+1][workingBuffer] ; 
