...
void TZXLoop() {   
 ...
 //       wbuffer[workingBuffer ^ 1][btemppos] = currentPeriod /256;   //add period to the buffer
        //wbuffer[btemppos][workingBuffer ^ 1] = highByte(currentPeriod);
       // wbuffer[btemppos][workingBuffer ^ 1] = (currentPeriod & 0xFF00) >>8 ;
 ...
 //       wbuffer[workingBuffer ^ 1][btemppos+1] = currentPeriod & 0xFF ;  
        //*((word *) (wbuffer[workingBuffer ^ 1] + btemppos) ) = currentPeriod;
 //       *(wbuffer[workingBuffer ^ 1] + btemppos)  = currentPeriod /256;
 //       *(wbuffer[workingBuffer ^ 1] + btemppos+1)  = currentPeriod & 0xFF ;
 //       *((word *)(wbuffer[workingBuffer ^ 1] + btemppos)) = currentPeriod/256;
 //       *((word *)(wbuffer[workingBuffer ^ 1] + btemppos+1)) = currentPeriod%256;
        *(wbuffer[workingBuffer ^ 1] + btemppos) = (byte) (currentPeriod >>8);
        *(wbuffer[workingBuffer ^ 1] + btemppos+1) = (byte) (currentPeriod & 0xFF); //ahorra 4 bytes si va primero
        interrupts();
        //btemppos+=1;
        btemppos+=2;        
      }
...
void wave2() {
  ...
  //word workingPeriod = (wbuffer[pos][workingBuffer] <<8) | wbuffer[pos+1][workingBuffer];
  word workingPeriod = 256 * wbuffer[workingBuffer][pos] | wbuffer[workingBuffer][pos+1];
...
//  if(isStopped==0 && ( wbuffer[pos][workingBuffer] || wbuffer[pos+1][workingBuffer] ) >= 1)
  {
      if(bitRead(workingPeriod, 15) && !pskipMask)
   //   if(bitRead(wbuffer[pos][workingBuffer], 7) && !pskipMask)         
      {
        ...
        bitClear(wbuffer[workingBuffer][pos],7);         //Clear pause block flag
        pinState = !pinState;
        pauseFlipBit = true;
        wasPauseBlock = true;
      } else {
        if (workingPeriod >= 1 && wasPauseBlock==false) {
   //     if( ( wbuffer[pos][workingBuffer] || wbuffer[pos+1][workingBuffer] ) >= 1 && wasPauseBlock==false) {
          pinState = !pinState;
  ...
  //      wbuffer[pos][workingBuffer] = (workingPeriod - 1) /256;  //reduce pause by 1ms as we've already pause for 1.5ms
        //wbuffer[pos][workingBuffer] = ((workingPeriod - 1) & 0xFF00) >>8 ;
  ...
  //      wbuffer[pos+1][workingBuffer] = (workingPeriod - 1) & 0xFF ;
        if (wbuffer[workingBuffer][pos+1] > 0) {   
          //wbuffer[workingBuffer][pos+1] -=1;
          *(wbuffer[workingBuffer] +pos+1) = (byte) (*(wbuffer[workingBuffer] +pos+1) -1);              
        } else {
          if (wbuffer[workingBuffer][pos] > 0) {
            //wbuffer[workingBuffer][pos+1] = 255; //ahorra 4 bytes si va primero
            *(wbuffer[workingBuffer] +pos+1) = 255;
            //wbuffer[workingBuffer][pos] -=1;
            *(wbuffer[workingBuffer] +pos) = (byte) (*(wbuffer[workingBuffer] +pos) -1);               
          }
        }
        pauseFlipBit=false;
      } else {
    ...
    //      newTime = long(256 * wbuffer[pos][workingBuffer] | wbuffer[pos+1][workingBuffer])*1000; //Set pause length in microseconds
          //newTime = long(workingPeriod)*1;
          isPauseBlock = false;
        } else {
          newTime = workingPeriod;          //After all that, if it's not a pause block set the pulse period 
    //      newTime = 256 * wbuffer[pos][workingBuffer] | wbuffer[pos+1][workingBuffer]; //After all that, if it's not a pause block set the pulse period 
        }
        //pos += 1;
        pos += 2;
...
//  } else if( (wbuffer[pos][workingBuffer] <=1) && (wbuffer[pos+1][workingBuffer] == 0) && isStopped==0 ) {
...
void process()
{
  ...
  {
    if(lastByte != 0x55) writeByte(0x55);   
    if(!count==0) {
      writeSilence();
      count+=-1;  
    } else stopFile();    
  ...
  { 
    if(dragonMode==1) {
      ...
      for(int t=0;t<8;t++)
      {
        ...
        {
          wbuffer[working ^ 1][btemppos] = bits[t];
          btemppos+=1;         
        }        
      }
      interrupts();
