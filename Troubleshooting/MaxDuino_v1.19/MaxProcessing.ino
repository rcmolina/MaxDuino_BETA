...
void TZXLoop() {   
    ...
    if(btemppos<=buffsize){                    // Keep filling until full
      ...
      if(currentPeriod>0) {
        ...
        //wbuffer[btemppos][workingBuffer ^ 1] = (byte) (currentPeriod >>8) ;
        ...
        //wbuffer[btemppos+1][workingBuffer ^ 1] = currentPeriod & 0xFF ;   
        interrupts();
        //btemppos+=1;
        btemppos+=2;        
      }
...
//  if(isStopped==0 && ( wbuffer[pos][workingBuffer] || wbuffer[pos+1][workingBuffer] ) >= 1)
  {
   ...
   //   if(bitRead(wbuffer[pos][workingBuffer], 7) && !pskipMask)         
      {
        ...
        //bitClear(wbuffer[pos][workingBuffer],7);         //Clear pause block flag
        pinState = !pinState;
        pauseFlipBit = true;
        wasPauseBlock = true;
      } else {
   ...
   //     if( ( wbuffer[pos][workingBuffer] || wbuffer[pos+1][workingBuffer] ) >= 1 && wasPauseBlock==false) {
      ...
      if(pauseFlipBit==true) {
        ...
        //wbuffer[pos][workingBuffer] = workingPeriod - 1) >>8 ;
 ...
 /*       if (wbuffer[pos+1][workingBuffer] > 0) {   
          wbuffer[pos+1][workingBuffer] -=1;              
        } else {
          if (wbuffer[pos][workingBuffer] > 0) {
            wbuffer[pos+1][workingBuffer] = 255;
            wbuffer[pos][workingBuffer] -=1;
          }
        } */
        pauseFlipBit=false;
      } else {
