#define VERSION "MaxDuino 1.30 M"
// ---------------------------------------------------------------------------------
...
// =====================================================================================================================================
//
//          The "MaxDuino 1.30 M" version is a variation of the "v1.30 MaxDuino" base version before mentioned.
...
//            - On LCD 20x4 screens it shows selected options Baud (Baud rate), Motor (Motor Ctrl), PSkip (Skip Pauses) & TSXSp (TSXspeedup)
...
#define LCDSCREEN20x4  1
//#define OLED1306   1
//#define P8544   1

#define MENU_ON 1  //comment to remove menu and save space
#define CNTRBASE 100  // 100 for ss, 60 for m:ss
...
void loop(void) {
  if(start==1) {
    uniLoop();    // TZXLoop only runs if a file is playing, and keeps the buffer full.
  } else {
    WRITE_LOW;    // digitalWrite(outputPin, LOW);    //Keep output LOW while no file is playing.
  }
  ...
  if (millis() - timeDiff > 50) {   // check switch every 50ms 
     ...
     if(digitalRead(btnPlay) == LOW) {
        ...
        if(start==0) {
          ...
          #ifdef LCDSCREEN20x4
            if (currpct <100) {              
              itoa(newpct,PlayBytes,10);strcat_P(PlayBytes,PSTR("%"));lcd.setCursor(11,0);lcd.print(PlayBytes);
            }
            strcpy(PlayBytes,"000");
            if ((lcdsegs %(CNTRBASE*10)) <10) itoa(lcdsegs%10,PlayBytes+2,10);
              else 
