...
unsigned long scrollTime = mymillis() + scrollWait;
...
//  digitalWrite(btnRoot, HIGH); 
  PORTD |= _BV(btnRoot);
      ...
      {
        mydelay(100);              // Show logo (OLED) or text (LCD) and remains until a button is pressed.
      }   
      #ifdef OLED1306    
          reset_display();           // Clear logo and load saved mode
      #endif
...
void loop(void) {
  ...
  if((mymillis()>=scrollTime) && start==0 && (strlen(fileName)> SCREENSIZE)) {
    //Filename scrolling only runs if no file is playing to prevent I2C writes 
    //conflicting with the playback Interrupt
    scrollTime = mymillis()+scrollSpeed;
    scrollText(fileName);
    scrollPos +=1;
    if(scrollPos>strlen(fileName)) {
      scrollPos=0;
      scrollTime=mymillis()+scrollWait;
      scrollText(fileName);
    }
  }
  motorState=digitalRead(btnMotor);
  #if (SPLASH_SCREEN && TIMEOUT_RESET)
      if (mymillis() - timeDiff_reset > 1000) //check timeout reset every second
      {
        timeDiff_reset = mymillis(); // get current millisecond count
        if (start==0)
        {
          timeout_reset--;
          if (timeout_reset==0)
  ...
  if (mymillis() - timeDiff > 50) {   // check switch every 50ms 
     timeDiff = mymillis();           // get current millisecond count
...
/*       
       ...
       while ((digitalRead(btnUp)==LOW) && (mymillis() - timeDiff2 < 200)) {
         //prevent button repeats by waiting until the button is released.
         mydelay(50); 
       }        
*/       
...
/*
       timeDiff2 = mymillis();           // get current millisecond count  
       while ((digitalRead(btnDown)==LOW) && (mymillis() - timeDiff2 < 200)) {
         //prevent button repeats by waiting until the button is released.
         mydelay(50); 
       }
*/ 
...
void debouncemax(int boton){
  timeDiff2 = mymillis();
  while ((digitalRead(boton)==LOW) && (mymillis() - timeDiff2 < 200)) {
    //prevent button repeats by waiting until the button is released.
    mydelay(50);
  }
}
